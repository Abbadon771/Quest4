...
void GUI::displayGameState(Samus* samus, Block** blocks, int numBlocks)
{
	...
	for (int i = 0; i<numBlocks; i++) {
		blockSheetTexture->render(renderer, blocks[i]->getPosX(), blocks[i]->getPosY(), NULL);
	}

  

    //Render Samus
	metroidSheetTexture->render(renderer, samus -> getPosX(), samus->getPosY(), &metroidClips[samus->getSpriteID()]);
	...
	SDL_Rect renderQuad = { SCREEN_WIDTH - 150, 10, width, height };
